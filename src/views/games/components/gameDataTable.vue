        ...
        <div class="table_head">
            <ul>
                ...
                <li v-for="(item,i) in BestHead" :key="i" class="table_head_playtype">
                    <span>{{ item | bestheadfilte(item.WagerTypeID,item.WagerGrpID) }}</span>
                </li>
            </ul>
        </div>
        <div class="table_main">
            <el-collapse v-model="activeName" class="include" @change="handleChange">
                <el-collapse-item v-for="(item,i) in List" :key="i" :name=i class="table_team_list">
                    <template slot="title">
                        <i class="el-icon-arrow-up"></i>
                        {{ item.LeagueNameStr }}
                    </template>
                    <table width="100%">
                        <!-- 队伍层-->
                        <tr v-for="(item_team,y) in item.Team" v-show="item_team.EvtStatus === 1" :key="y"
                            class="list_table_tr">
                            ...
                            <td v-for="(item_team_Wager,e) in filterdata(item_team.Wager)" :key="e"  class="odds_td">
                                <ul v-if="item_team_Wager.notdata || item_team_Wager.Odds[0].Status!==1">
                                    <li></li>
                                    <li></li>
                                    <li v-if="item_team.isDrewOdds" ></li>
                                </ul>
                                <ul v-else-if="item_team_Wager.WagerTypeID === 101 || item_team_Wager.WagerTypeID === 103"
                                    class="odds_td_ul">
                                    <li class="havedata" @click="gobet({
                                ...
                                <ul v-else-if="item_team_Wager.WagerTypeID === 102 || item_team_Wager.WagerTypeID === 104 || item_team_Wager.WagerTypeID === 109"
                                    class="odds_td_ul">
                                    <li class="havedata" @click="gobet({
                                     type:item.CatID!==4?0:1,
                                     WagerPos:4,
                                ...
                                <ul v-else-if="item_team_Wager.WagerTypeID === 111 || item_team_Wager.WagerTypeID === 110"
                                    class="odds_td_ul">
                                    <li class="havedata" @click="gobet({
                                     type:item.CatID!==4?0:1,
                                     CatID:item.CatID,
                                     ...
                                     EvtID:item_team.EvtID,
                                        ...
                                        <span v-if="item.CatID!==4"
                                              class="odds_center">{{ item_team_Wager.Odds[0].HomeOdds }}</span>
                                        <span v-else class="odds_center">{{ item_team_Wager.Odds[0].AwayOdds }}</span>
                                    </li>
                                    <li class="havedata" @click="gobet({
                                     type:item.CatID!==4?1:0,
                                     CatID:item.CatID,
                                     ...
                                     EvtID:item_team.EvtID,
                                        ...
                                        <span v-if="item.CatID!==4"
                                              class="odds_center">{{ item_team_Wager.Odds[0].AwayOdds }}</span>
                                        <span v-else class="odds_center">{{ item_team_Wager.Odds[0].HomeOdds }}</span>
                                    </li>
                                    <li v-if="item_team.isDrewOdds" :class=" item_team_Wager.Odds[0].DrewOdds === 0?'':'havedata'" @click="gobet({
                                     type:2,
                                     CatID:item.CatID,
                                     WagerTypeID:item_team_Wager.WagerTypeID,
                                     WagerGrpID:item_team_Wager.WagerGrpID,
                                     ...
                                     EvtID:item_team.EvtID,
                                        ...
                                        {{
                                            item_team_Wager.Odds[0].DrewOdds === 0 ? '' : item_team_Wager.Odds[0].DrewOdds
                                        }}
                                    </li>
                                </ul>
            ...
            <div v-show="List.length<=0" class="notdata">
                <div>
                    <img alt="" src="https://dcd.nbb21n.net/images/bb/white/icon_noGame.svg">
                    <p>暫無賽事</p>
                </div>
...
export default {
    ...
    filters: {
        time_md(val) {
            ...
            var timeYMD = time.replace(" ", ":").replace(/\:/g, "/").split("/");
            return timeYMD[1] + '-' + timeYMD[2]
        },
        time_hm(val) {
            var time = val.split(' ')[1];
            var timeHM = time.replace(" ", ":").replace(/\:/g, "/").split("/");
            return timeHM[0] + ':' + timeHM[1]
        }
    },

    methods: {
        ...mapMutations('games', ['ADD_BERDATA', 'SET_TABLEDATA', 'SET_CADID','SET_TAB','SET_KEY']),
        gobet(data) {
            this.SET_TAB(0)
            const {
                type,
            ...
            if (WagerTypeID === 101 || WagerTypeID === 103) {
                ...
                ds = type === 0 ? Odds.HomeHdpOdds : (type === 1 ? Odds.AwayHdpOdds : Odds.DrewOdds)
            } else if (WagerTypeID === 102 || WagerTypeID === 104 || WagerTypeID === 109) {
                Hdp = Odds.OULine
            ...
            var WagerString = CatID + ',' + Odds.GameID + ',' + WagerTypeID + ',' + WagerGrpID + ',' + WagerPos + ',' + Odds.HdpPos + ',' + Hdp + ',' + ds + ',' + 'DE'
            ...
            console.log(this.getBetdata.length);
        },
        handleChange() {
        ...
        filterdata(list) {
            ...
            for (var i = 0; i < this.BestHead.length; i++) {
                for (var y = 0; y < list.length; y++) {
                    if (this.BestHead[i].WagerGrpID === list[y].WagerGrpID && this.BestHead[i].WagerTypeID.indexOf(list[y].WagerTypeID) > -1) {
                        data[i] = list[y]
                    }
                }
            }
        ...
        getGameDetail(key) {
            getGameDetail({lang: 'cn', CatID: this.getCadID, GameType: this.getgameState,WagerTypeKey:key}).then((res) => {
                console.log("右侧玩法数据:", res);
                    res.data.List.forEach((item,i) => {
                        item.Team.forEach((item_Team,t) => {
                            console.log(item_Team.Row);
                            // if(item_Team.Row>1){
    ...
    computed: {
        ...mapGetters('games', ['getCadID', 'getBestHead', 'getBestList', 'getBetdata', 'getgameState','getKey']),
    },
    mounted() {
        var _this = this
        this.getGameDetail(1)
