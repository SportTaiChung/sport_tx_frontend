<template>
  <div
    class="betInfo_Center"
    v-loading="loading"
    element-loading-background="rgba(0, 0, 0, 0.8)"
    v-if="getBetdata.length > 0"
  >
    <div v-for="(item, i) in getBetdata" :key="i" class="betInfo_List">
      <div
        class="BIL_In"
        :class="getclass(item.Odds.Status, item.isrepeat, item.beterror)"
      >
        <div class="flex BIL_T_text">
          <div>
            <span class="t_blue"> {{ item.bet_title }} </span>
            <span class="t_red">{{ item.Cutline }}</span>
            <span class="t_inlineBox">
              @ <span class="t_red">{{ item.OddsValue }}</span></span
            >
          </div>
        ...
        <div class="BIL_In_padding">
          ...
          <div>
            {{ item.HomeTeamStr }} <span class="t_orange">(主)</span> vs
            {{ item.AwayTeamStr }}
          </div>
          <div v-show="type === 0" class="BIL_input">
            ...
            <input
              placeholder="可赢金額"
              :value="Math.floor(item.Amount * item.OddsValue)"
    ...
    <div v-show="type === 0" class="betPlay_list flex">
      <span>單 注:</span>
      <div>
        <input v-model="allAmount" placeholder="" type="Number" />
    ...
    <div class="betPlay_list flex betPlay_list">
      ...
      <div v-show="type === 1">
        <input
          ...
          placeholder=""
          :value="
            Math.floor(getBetdata[0].stray * AmountS * 100) / 100 - AmountS
          "
          style="width: 100%;"
          type="number"
        />
    ...
    <div class="betPlay_btm">
      ...
      <ul class=" color_text" v-show="type === 1">
        ...
        <li class="flex">
          ...
          <div class="BPBM_R">
            {{
              Math.floor(AmountS * getBetdata[0].stray * 100) / 100 - AmountS
            }}
          </div>
        </li>
      </ul>
          ...
          this.updataInterval = setInterval(async function() {
            await getBetInfo({ GameIDs: ids }).then((res) => {
              res.data.forEach((item, i) => {
                if (
                  _this.getBetdata[i].WagerTypeID === 101 ||
                  _this.getBetdata[i].WagerTypeID === 103
                ) {
                  ...
                  _this.getBetdata[i].WagerTypeID === 102 ||
                  _this.getBetdata[i].WagerTypeID === 104 ||
                  _this.getBetdata[i].WagerTypeID === 109
                ) {
                  if (_this.getBetdata[i].type === 0) {
                  ...
                  _this.getBetdata[i].WagerTypeID === 110 ||
                  _this.getBetdata[i].WagerTypeID === 111
                ) {
                  if (_this.getBetdata[i].type === 0) {
