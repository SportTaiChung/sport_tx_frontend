  ...
  <div class="rightBody color_text ">
    <div>
      <ul class="flex rightBody_head">
        <li :class="getthistab === 0 ? 'is_active' : ''" @click="SET_TAB(0)">
          投注資訊
        </li>
        <li :class="getthistab === 1 ? 'is_active' : ''" @click="SET_TAB(1)">
          最新注單{{ getHistorydata.length <= 0 ? "" : getHistorydata.length }}
        </li>
      </ul>
    </div>
    <div v-show="getthistab === 0" class="rightBody_main">
      <div v-show="getBetdata.length > 0">
        <ul class="flex betInfo_SubT">
          <li
            :class="activeBetInfo === 0 ? 'Tou_active' : ''"
            @click="selcetBetInfo(0)"
          >
            單項投注{{ getBetdata.length }}
          </li>
          <li
            :class="activeBetInfo === 1 ? 'Tou_active' : ''"
            @click="selcetBetInfo(1)"
          >
            過關投注{{ getBetdata.length }}
          </li>
        </ul>
        ...
        <div
          ...
          style="color: red;text-align: center;line-height: 100px"
        >
          過關投注請至少選擇2場賽事
        </div>
      </div>
      <div
        v-show="getBetdata.length <= 0"
        style="color: red;text-align: center;line-height: 100px"
      >
        请先点击赔率以添加选项
      </div>
    ...
    <div v-show="getthistab === 1" class="rightBody_main">
      <div>
        <ul class="flex betInfo_SubT">
          <li
            :class="activeNewbet === 0 ? 'Tou_active' : ''"
            @click="selcetNewbet(0)"
          >
            未结算注單
            <span class="t_blue">{{
              getHistorydata.length <= 0 ? "" : getHistorydata.length
            }}</span>
          </li>
          <li
            :class="activeNewbet === 1 ? 'Tou_active' : ''"
            @click="selcetNewbet(1)"
          >
            已结算注單
            <span class="t_change">{{
              getHistorySettled.length <= 0 ? "" : getHistorySettled.length
            }}</span>
          </li>
        </ul>
...
.rightBody_main::-webkit-scrollbar {
  width: 0;
}
.is_active {
  color: #fff;
