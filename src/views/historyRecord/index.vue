      ...
      <table v-show="active === 0" border="0" cellspacing="0" cellpadding="0">
        ...
        <tr class="rt_data" v-for="(item, i) in getbetHistorydata">
          ...
          <td class="rt_fs" v-if="item.BetType === 99">
            <div
              class="rt_fs_list"
              v-for="(betlist, y) in item.dataBet"
              :key="y"
            >
              <div>{{ y + 1 }}</div>
              <ul>
          ...
          >
            <tr>
              <td width="585" @click="upactive(item.CatID)">
                <i
                  :class="
                    item.active ? 'el-icon-arrow-down' : 'el-icon-arrow-up'
                  "
                ></i>
                {{ item.catName }}
              </td>
          ...
          >
            <tr class="rt_data">
              <td width="185" class="rt_info">
                <ul>
                  ...
                  <li v-if="itemdata.BetType === 1">
                    <span>比赛 : </span>
                    <span>{{
                      itemdata.dataBet[0].ScheduleTimeStr.slice(5, 16)
                    }}</span>
                  </li>
                  <li v-else>
                    <span>类型 : 過關 </span>
              ...
              <td width="400" class="rt_fs" v-if="itemdata.BetType === 99">
                <div
                  class="rt_fs_list"
                  v-for="(betlist, y) in itemdata.dataBet"
                  :key="y"
                >
                  <div>{{ y + 1 }}</div>
                  <ul>
                    ...
                    <li>
                      {{ betlist.HomeTeam }} (主) VS {{ betlist.AwayTeam }}
                    </li>
                    <li>
                      投注： {{ betlist.betname }} {{ betlist.CutLine }} @
                      {{ betlist.PayoutOddsStr }}
              ...
              <td width="100" class="rt_betval" v-if="itemdata.BetType === 1">
                {{
                  Math.floor(
                    itemdata.Amount * itemdata.dataBet[0].PayoutOddsStr
                  )
                }}
              </td>
      ...
      this.betHistorydata.forEach((item, i) => {
        if (item.BetType === 1) {
          if (item.dataBet[0].WagerPosName === "主隊") {
            this.betHistorydata[i].dataBet[0].betname =
          ...
          item.dataBet.forEach((Betlist, y) => {
            if (Betlist.WagerPosName === "主隊") {
              ...
              this.betHistorydata[i].dataBet[y].betname =
                item.dataBet[y].WagerPosName;
            }
          });
        }
      });
