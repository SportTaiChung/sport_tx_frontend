<template>
  <div>
    <div class="nav_header" v-show="!isCollapse">
      <ul>
        <li
          v-for="(item, i) in gameState"
          :key="i"
          v-if="i < 3"
          @click="getstate(item.key)"
          :class="state === item.key ? 'activestate' : ''"
        >
          {{ item.value }}
        </li>
      </ul>
    ...
    >
      <el-submenu index="状态" class="collapse_GameType" v-show="isCollapse">
        ...
        <el-menu-item-group>
          <el-menu-item v-for="(item, i) in gameState" :key="i">{{
            item.value
          }}</el-menu-item>
        </el-menu-item-group>
      </el-submenu>
      <el-submenu v-for="(item, i) in ballType" :key="i" :index="i + ''">
        <template slot="title">
          ...
          <div class="flex nav_bottom" @click="selectmenu(item.catid, 1)">
        ...
        <el-menu-item-group v-show="item.Items.length > 0">
          <el-menu-item
            v-for="(Items, y) in item.Items"
            :key="y"
            @click="selectmenu(item.catid, Items.WagerTypeKey)"
            :index="i + '-' + y"
          >
            <template slot="title">
...
export default {
  ...
  methods: {
    ...
    getmenudata(pid) {
      getMenu({ lang: "cn", gtype: this.getgameState, pID: pid }).then(
        (res) => {
